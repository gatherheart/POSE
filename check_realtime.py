...
def parseImg(arg, identity):
    try:
        ...
        while not q.empty():
            ...
            print("Done " + image) 
            with open('/home/ubuntu/pythonServer/parsed/'+identity+'/json/'+image.split('.')[0]+'.json', 'w') as f:
                cv2.imwrite("/home/ubuntu/pythonServer/parsed/"+identity+"/image/"+image, datum.cvOutputData)
                json.dump(data, f)
